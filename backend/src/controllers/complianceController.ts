...
import { calculateComplianceStatus } from '../services/complianceStatus.js';
...
function normalizeDocumentType(value: unknown): string | undefined {
	return DOCUMENT_TYPE_ALIASES[
		String(value || '').trim().toUpperCase() as keyof typeof DOCUMENT_TYPE_ALIASES
	];
}
...
function validateCompliancePayload(
	payload: any,
	...
	const documentNumber =
		payload.document_number === undefined
			? undefined
			: String(payload.document_number).trim();
	const leadTimeDays =
	...
	if (creating && !payload.expiration_date) {
		errors.push('Expiration date is required.');
	}
	if (
		payload.expiration_date !== undefined &&
		!isValidDate(payload.expiration_date)
	) {
		errors.push('Expiration date must use the YYYY-MM-DD format.');
	}
	if (
...
async function createComplianceDocument(
	...
	try {
		...
		if (vehicleError) {
			return res.status(500).json({ error: 'Unable to verify vehicle.', details: vehicleError.message });
		}
		...
		if (existingError) {
			return res.status(500).json({ error: 'Unable to verify compliance document.', details: existingError.message });
		}
		if (existing) {
			return res.status(409).json({ error: `A ${data.document_type} document already exists for this vehicle. Update it instead.` });
		}

		const complianceItem: any = {
			...data,
			lead_time_days: data.lead_time_days ?? 30
		};
		complianceItem.status = calculateComplianceStatus(complianceItem);
		if (complianceItem.document_number === undefined) {
			delete complianceItem.document_number;
		}
		...
		if (insertError) {
			return res.status(500).json({ error: 'Unable to create compliance document.', details: insertError.message });
		}
...
async function updateComplianceDocument(
	req: RequestWithSupabase,
	res: Response
): Promise<Response> {
	try {
